...
func LaunchDevnetDocker(t *testing.T, numMiners int) *dockertest.Resource {
	...
	lotusDevnet, err := pool.RunWithOptions(&dockertest.RunOptions{Repository: "textile/lotus-devnet", Tag: "sha-d0ecbd8", Env: []string{envNumMiners, speed}, Mounts: []string{"/tmp/powergate:/tmp/powergate"}})
	if err != nil {
